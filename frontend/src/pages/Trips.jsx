...
export default function Trips() {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([tripsApi.list(filterStatus ? { status: filterStatus } : {}), vehiclesApi.list({ status: "AVAILABLE" }), driversApi.list({ status: "ON_DUTY" })])
      .then(([t, v, d]) => { setData(t.data); setVehicles(v.data); setDrivers(d.data); })
  ...
  const handleDispatch = async () => {
    ...
    setError("");
    try {
      await tripsApi.create({ ...form, cargoWeight: Number(form.cargoWeight), distanceKm: Number(form.distanceKm), revenue: Number(form.revenue) });
      setModal(false);
      load();
    } catch (e) {
  ...
  const handleComplete = async () => {
    ...
    setError("");
    try {
      await tripsApi.complete(completeModal.id, { ...completeForm, endOdometer: Number(completeForm.endOdometer), distanceKm: Number(completeForm.distanceKm), revenue: Number(completeForm.revenue) });
  ...
  const handleCancel = async (id) => {
    ...
    try { await tripsApi.cancel(id); load(); } catch (e) { setError(e.response?.data?.error || "Cancel failed"); }
  };
  ...
  return (
    <div className="fade-in">
      <PageHeader
        ...
        actions={<Button onClick={() => { setForm(emptyForm); setError(""); setModal(true); }}><Plus size={14} />Dispatch Trip</Button>}
      />
              ...
              <tbody>
                {data.length === 0 ? (
                  ...
                  <tr key={t.id} className="border-b border-surface-border/50 hover:bg-surface-hover/30 transition-colors">
                    ...
                    <td className="py-3 px-4 text-text-primary text-xs font-mono">{t.vehicle?.name}</td>
                    ...
                    <td className="py-3 px-4 font-mono text-emerald-400 text-xs">₹{t.revenue?.toLocaleString()}</td>
                    <td className="py-3 px-4"><StatusPill status={t.status} /></td>
                    <td className="py-3 px-4 font-mono text-text-muted text-xs">{format(new Date(t.createdAt), "dd/MM/yy")}</td>
                    <td className="py-3 px-4">
                      <div className="flex gap-2">
                        {t.status === "DISPATCHED" && (
                          <>
                            <button onClick={() => { setCompleteForm({ endOdometer: "", distanceKm: t.distanceKm, revenue: t.revenue }); setCompleteModal(t); }} className="text-xs font-mono text-emerald-400 hover:text-emerald-300">Complete</button>
                            <button onClick={() => handleCancel(t.id)} className="text-xs font-mono text-red-400 hover:text-red-300">Cancel</button>
                          </>
                        )}
                        {t.status === "DRAFT" && <button onClick={() => handleCancel(t.id)} className="text-xs font-mono text-red-400 hover:text-red-300">Cancel</button>}
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="space-y-4">
          {completeModal && (
            <div className="bg-surface-hover rounded-lg p-3 text-xs font-mono text-text-secondary">
              <p className="text-text-primary font-700">{completeModal.vehicle?.name} → {completeModal.driver?.name}</p>
